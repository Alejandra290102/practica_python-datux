...
def IngestDataProducts(app:App):
    ...
    dataMarket=GetDataSourceMarket()
    CreateTableMarket(conn)
    InsertDataMarket(bd,dataMarket)
...
def GetDataSourcePais():
    pathData="/workspaces/practica_python-datux/proyecto/files/datafuente.xls"
...
def GetDatasourceOrders(conn):
    ...
    df_orders=df[['Order ID','Postal Code','Product ID','Sales','Quantity','Discount','Profit','Market','Shipping Cost','Order Priority']].dropna().drop_duplicates()
    ...
    df_newOrders=df_newOrders[['Order ID','Postal Code','id','Sales','Quantity','Discount','Profit','Market','Shipping Cost','Order Priority']]
    list_tuples=[tuple(x) for x in df_newOrders.to_records(index=False)]
    return list_tuples


def GetDataSourceMarket():
    pathData="/workspaces/practica_python-datux/proyecto/files/datafuente.xls"
    df=pd.read_excel(pathData,sheet_name="Orders")
    print(df.shape)
    print(df.keys())
    df_Market=df['Market'].unique()
    print(df_Market.shape)
    Market_tuples = [(Market,) for Market in df_Market] 
    
    return Market_tuples

def CreateTableMarket(conn:Connection):
    market=Market()
    market.create_table(conn)

def InsertDataMarket(bd:Database,data):
    bd.insert_many('MARKET',['name'],data)
...
def insertManyVentas(bd:Database,data):
    bd.insert_many('VENTAS',['order_id','postal_code','product_id','sales_amount','quantity','discount','profit','market', 'shipping_cost','order_priority'],data)